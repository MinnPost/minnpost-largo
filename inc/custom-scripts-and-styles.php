...
if ( ! function_exists( 'minnpost_largo_add_remove_styles' ) ) :
	...
	function minnpost_largo_add_remove_styles() {
		...
		if ( is_single() ) {
			...
			$css_url = get_post_meta( get_the_ID(), '_css_file_url', true );
			if ( '' !== $css_url ) {
				$css_version = get_post_meta( get_the_ID(), '_css_file_version', true );
				if ( '' === $css_version ) {
					$css_version = false;
				}
...
if ( ! function_exists( 'minnpost_largo_add_remove_scripts' ) ) :
	...
	function minnpost_largo_add_remove_scripts() {
		...
		if ( is_single() ) {
			$js_url = get_post_meta( get_the_ID(), '_js_file_url', true );
			if ( '' !== $js_url ) {
				$js_version = get_post_meta( get_the_ID(), '_js_file_version', true );
				if ( '' === $js_version ) {
					$js_version = false;
				}
