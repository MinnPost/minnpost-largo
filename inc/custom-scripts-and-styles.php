...
if ( ! function_exists( 'minnpost_largo_remove_scripts' ) && ! function_exists( 'minnpost_largo_add_remove_styles' ) ) :
	add_action( 'wp_enqueue_scripts', 'minnpost_largo_remove_scripts', 10 );
	...
	function minnpost_largo_add_remove_styles() {
		...
		wp_enqueue_style( 'minnpost-style', get_theme_file_uri() . '/style' . $suffix . '.css', array(), filemtime( get_template_directory() . '/style' . $suffix . '.css' ), false );
